...
func calculateArea(x : Double, y : Double) -> Double?{
    // dont allow for negative values
    guard x > 0 && y > 0 else {
...
func add(alpha : Int?, beta : Int?) -> Int?{
    guard let a : Int = alpha, let b :Int = beta else{
        print("one ir more od th values given are nil")
        return nil
    }
    return a + b
}
...
func createUser() -> User? {
    ...
    var user = User (firstName,lastName: last, age: age)
    return user
}
...
var dude = createUser()
if let user = dude{
    print("The user name is \(user.firstName) \(user.lastName), and they are \(user.age) years old")
}
/*:
